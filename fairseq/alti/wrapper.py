...
import fairseq.model_parallel.models.pipeline_parallel_transformer
...
class AltiHubInterface(GeneratorHubInterface):
    ATTN_MODULES = ['encoder.self_attn',
                    'decoder.self_attn',
                    'decoder.encoder_attn']

    def __init__(self, cfg, task, models, pipeline=False):
    ...
    def __get_attn_weights_module(self, layer_outputs, module_name):
        ...
        layer_prefix = get_layer_prefix(enc_dec_, l, self.pipeline)

        k = layer_outputs[f"{layer_prefix}.{attn_module_}.k_proj"][0]
        q = layer_outputs[f"{layer_prefix}.{attn_module_}.q_proj"][0]
    ...
    def __get_contributions_module(self, layer_inputs, layer_outputs, contrib_type, module_name):
        ...
        eps_ln2 = ln.eps
        layer_prefix = get_layer_prefix(enc_dec_, l, self.pipeline)
        in_q = layer_inputs[f"{layer_prefix}.{attn_module_}.q_proj"][0][0].transpose(0, 1)
        in_v = layer_inputs[f"{layer_prefix}.{attn_module_}.v_proj"][0][0].transpose(0, 1)
        in_res = layer_inputs[f"{layer_prefix}.{attn_module_}_layer_norm"][0][0].transpose(0, 1)
        ##
        in_res2 = layer_inputs[f"{layer_prefix}.final_layer_norm"][0][0].transpose(0, 1)
        ...
        if pre_layer_norm==False:
            ...
            out_q_pre_ln_th = layer_inputs[f"{layer_prefix}.{attn_module_}_layer_norm"][0][0].transpose(0, 1)
        else:
            if 'encoder' in enc_dec_:
                # Encoder (self-attention) -> final_layer_norm
                out_q_pre_ln_th = layer_inputs[f"{layer_prefix}.final_layer_norm"][0][0].transpose(0, 1)
            else:
                if "self_attn" in attn_module_:
                    # Self-attention decoder -> encoder_attn_layer_norm
                    out_q_pre_ln_th = layer_inputs[f"{layer_prefix}.encoder_attn_layer_norm"][0][0].transpose(0, 1)
                else:
        ...
        if pre_layer_norm:
            ...
            out_q_th_2 = layer_outputs[f"{layer_prefix}.{attn_module_}_layer_norm"][0].transpose(0, 1)
    ...
    def get_contributions(self, src_tensor, tgt_tensor, contrib_type='l1', norm_mode='min_sum'):
        ...
        for attn in self.ATTN_MODULES:
            ...
            for l in range(len(get_layers(enc_dec))):
                if contrib_type == 'attn_w':
        ...
        devices = get_devices(contributions_all.values())
        # Find the first device in the list of devices
        device = next(iter(devices), "cpu")
        contributions_all = set_devices(contributions_all, "cpu")
...
def get_layers(enc_dec):
    if isinstance(enc_dec, fairseq.model_parallel.models.pipeline_parallel_transformer.model.TransformerEncoder):
        layers = enc_dec.model.partitions[0]
        # Don't return encodings and layer normalization (first and last layers layer)
        return layers[1:-1]
    if isinstance(enc_dec, fairseq.model_parallel.models.pipeline_parallel_transformer.model.TransformerDecoder):
        layers = []
        for p in enc_dec.model.partitions:
            layers.extend(p)
        # Don't return encodings and layer normalization (first and last layers layer)
        return layers[1:-1]
    return enc_dec.layers

def get_layer_prefix(enc_dec: str, layer_num: int, pipeline: bool):
    if pipeline:
        partition = get_layer_partition(enc_dec, layer_num)
        # Offset by 1 to skip the encoding layer
        return f'models.0.{enc_dec}.model.partitions.{partition}.{layer_num+1}'
    return f'models.0.{enc_dec}.layers.{layer_num}'

def get_layer_partition(enc_dec: str, layer_num: int):
    if enc_dec == 'encoder':
        return 0
    """"
...
def get_devices(contributions):
    devices = []
    for contributions_for_type in contributions:
        devices.extend([t.device for t in contributions_for_type])
    return devices

def set_devices(contributions_all, device):
    on_device = {}
    for k, contributions_for_type in contributions_all.items():
        on_device[k] = [t.to(device) for t in contributions_for_type]
    return on_device