...
from fairseq.alti.utils import make_batches
...
class AltiMultilingualHubInterface(AltiHubInterface):

    def __init__(self, cfg, task, models, pipeline=False):
        super().__init__(cfg, task, models, pipeline)
        self.eval()
        # self.to("cuda" if torch.cuda.is_available() else "cpu")
    
    @classmethod
    def from_pretrained(cls, checkpoint_dir, checkpoint_file, data_name_or_path,
                         source_lang, target_lang, lang_pairs, fixed_dictionary, pipeline, **kwargs):
        ...
        return cls(hub_interface.cfg, hub_interface.task, hub_interface.models, pipeline=pipeline)
    ...
    def trace_forward(self, src_tensor, tgt_tensor):
        ...
        with torch.no_grad():
            ...
            src_lengths = torch.tensor([src_tensor.size(-1)]).to(self.device)
            ...
            model_output, encoder_out = self.models[0](src_tensor, src_lengths, tgt_tensor, )
